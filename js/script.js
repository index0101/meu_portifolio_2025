...
const portfolioData = [
    ...
    },
    {
        ...
        details: {
            ...
            images: ["images/graphic_design_example_new2.jpg", "images/graphic_design_example_new1.jpg"]
        }
    }
];
    ...
    createModalContent(item) {
        return `
            ...
            <div class="modal-images">
                ${item.details.images.map(img => `<img src="${img}" alt="${item.title}" style="width:100%; margin-bottom:1rem; border-radius:0.75rem;">`).join('')}
            </div>
...
class ContactForm {
    ...
    init() { contactForm.addEventListener('submit', e => this.handleSubmit(e)); }
...
class App {
    ...
    init() {
        if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', () => this.initializeComponents());
        else this.initializeComponents();
    }
    ...
    initModalEvents() {
        ...
        document.addEventListener('keydown', e => { if(e.key==='Escape' && modal.classList.contains('active')) this.portfolio.closeModal(); });
    }
}
...
// Start App
const app = new App();
document.addEventListener('DOMContentLoaded', () => { portfolio = app.portfolio; });
...
@media(max-width:768px) { .detail-grid { grid-template-columns:1fr; gap:1rem; } }
</style>`;
